...
void GameMain::Create()
{
	...
	//デバックテキストのインスタンス生成.
	m_pDbgText = new DebugText();



	//スタティックメッシュのインスタンス生成.
	m_pStaticMeshFighter	= new StaticMesh();
	m_pStaticMeshGround		= new StaticMesh();
	m_pStaticMeshEnemyA		= new StaticMesh();
	m_pStaticMeshEnemyB		= new StaticMesh();
	m_pStaticMeshBullet		= new StaticMesh();
	m_pStaticMeshBSohere	= new StaticMesh(); // BSphere用のメッシュインスタンスをここで生成

	//スタティックメッシュオブジェクトクラスのインスタンス生成.
	m_pStcMeshObj = new StaticMeshObject();

	//スプライトのインスタンス作成.
	m_pSpriteGround		= new Sprite3D();
	m_pSpriteExplosion	= new Sprite3D(); // 爆発マネージャーに渡すためのスプライト

	//地面スプライトの構造体
	Sprite3D::SPRITE_STATE SSGround;
	SSGround.Disp.w = 1.f;
	SSGround.Disp.h = 1.f;
	...
	// 爆発スプライトの構造体
	Sprite3D::SPRITE_STATE SSExplosion =
	{ 1.f, 1.f,			// Disp(w.h)
